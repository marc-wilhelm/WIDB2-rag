...
- PDF-Dateien werden eingelesen und zunächst in [Markdown-Dateien](../data/) (.md) konvertiert.

### 1.2 Ergänzende Wissensquellen

- Jedem Dokument wird manuell ein zusätzlicher Abschnitt „[Fazit](../data/Geschäftsentwicklung_HomeTech.md#fazit-business-unit-home-tech-januar---april-2023)“ hinzugefügt, um eine einheitliche inhaltliche Zusammenfassung sicherzustellen und die spätere Retrieval-Qualität zu erhöhen.

- [Beispielfragen](../streamlit/pages/1_Chat.py#L34) in das Frontend eingefügt, um Nutzern eine Idee zu geben, welche Themen sie behandeln können und qualitätsgeprüfte Anfragen stellen können.
...
- Der bereinigte Text wird anhand von Überschriften und Abschnitten in kleinere, in sich geschlossene [Textsegmente](../src/MarkdownCleaner.py#L74) aufgeteilt.
...
- Jeder Textabschnitt wird mit [Metainformationen](../src/VectoreStoreManager.py#L50) versehen, z. B.:
	...
	- Paragraphen Typ (Einleitung, Monatsbericht)
...
- Die unstrukturierten Textsegmente werden gemeinsam mit ihren Metadaten und IDs mittels eines [Embedding-Modells](../src/VectoreStoreManager.py#L16) in Vektoren überführt.

- Eingesetztes Modell:

	-  [``` paraphrase-multilingual-MiniLM-L12-v2 ```](../src/config.py#L35)

- Dieses Modell erlaubt eine mehrsprachige semantische Repräsentation der Inhalte.

### 3.2 Speicherung in der Vektordatenbank

- Die erzeugten Vektoren werden in der [ChromaDB](../chroma_db/) gespeichert.
...
- Eingesetzte KI-Version:
	...
	- Claude Sonnet 4: [```claude-sonnet-4-20250514```](../src/config.py#L38)
...
- Ein zentraler [System-Prompt](../data/system_prompt.txt) definiert verbindliche Regeln für den Umgang mit Quellen, z. B.:
...
- Dabei gelten feste Einschränkungen:
	...
	- Maximale [Anzahl](../src/config.py#L43) der übergebenen Textsegmente (Absätze)

	- Maximale [Antwortlänge](../src/config.py#L39) in Tokens

Diese Begrenzungen dienen der Kostenkontrolle, Antwortqualität und Systemstabilität.

## 6. Frontend und Benutzerinteraktion

- Das Frontend besteht aus einer mit Streamlit umgesetzten [Webanwendung](../streamlit/).
...
- Falls eine Benutzeranfrage explizit die Erstellung einer Grafik oder Visualisierung verlangt:
	...
	- Greift die KI auf ein separates Modul [```grafik_plotten_dynamisch```](../src/Grafikplotter.py#L7) zu.

	- Dieses Modul wird mit anfragerelevanten Daten [parametrisiert](../src/RagAgent.py#L433) und erzeugt die gewünschte Grafik.
